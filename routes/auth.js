...
router.get('/google/callback', async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      // Update credits if missing for existing users
      if (!user.credits) {
        update.credits = 1000;
      }
    ...
    const token = jwt.sign(
      { userId: user._id, email: user.email, role: user.role, credits: user.credits },
...
router.post('/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { userId: user._id, email: user.email, role: user.role, credits: user.credits || 1000 },
      process.env.JWT_SECRET,
      { expiresIn: '7d' }
    );
